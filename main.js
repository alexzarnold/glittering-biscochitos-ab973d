...
(function () {
        ...
        mount.innerHTML =
          '<div class="' + boxClass + '">' +
            ...
            '<p class="tiny" style="margin:1.25rem 0 0">' +
              'The full schedule goes up once rooms are confirmed. ' +
              '<a class="link" href="join.html">Join the mailing list</a> ' +
      ...
      if (!upcoming.length) {
        upcomingMount.innerHTML =
          '<div class="empty-state">' + FIN_SVG +
            '<h3>The schedule is not posted yet</h3>' +
            '<p>Dates go up as soon as rooms are confirmed for [[SEMESTER_DATES]]. ' +
            'The first meeting is [[MEETING_DAY_TIME]] in [[BUILDING_ROOM]].</p>' +
            '<p><a class="link" href="join.html">Join the mailing list</a> ' +
            'and you will get every date as it is set.</p>' +
          '</div>';
      } else {
  ...
  function renderTeam() {
    ...
    if (!board.length) {
      mount.innerHTML =
        '<div class="empty-state">' + FIN_SVG +
          '<h3>The board is being confirmed</h3>' +
          '<p>Names go up shortly. Questions to ' +
          '<a class="link" href="mailto:orca@colorado.edu">orca@colorado.edu</a>.</p>' +
        '</div>';
      return;
    }
